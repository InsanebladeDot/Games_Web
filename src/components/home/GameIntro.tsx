...
export default function GameIntro({ title, description }: GameIntroProps) {
  // 用于同步左右两侧高度
  ...
  // 如果没有分割出段落，或者 description 为空，使用完整的 description 作为一段
  if (descriptionParagraphs.length === 0 && description) {
      descriptionParagraphs.push(description.trim());
  }
    // 如果 description 本身也是空的，显示一个默认文本
  ...
  return (
    <div className="w-full flex flex-col md:flex-row items-stretch bg-gradient-to-br from-gray-900 via-black to-gray-800 rounded-3xl shadow-2xl p-12 md:p-20 mt-12 max-w-7xl mx-auto">
      ...
      <div ref={leftRef} className="flex-1 pr-0 md:pr-12 flex flex-col justify-center">
        <h1 className="text-4xl md:text-5xl font-extrabold text-purple-200 mb-8 drop-shadow-lg">
          Welcome to {title}
        </h1>
        <p className="text-2xl text-gray-300 mb-5"> A battle royale rage game about sheep; will you have what it takes to survive in {title}? </p>
        ...
        <div className="flex flex-row gap-6 w-full relative z-10 mt-6"> {/* 保持用户当前的按钮布局类，调整 mt */}
          ...
          <button className="flex-1 bg-purple-800 hover:bg-purple-700 text-white font-bold py-4 px-10 rounded-2xl shadow-lg transition-all text-2xl whitespace-nowrap">
            DOWNLOAD CRAZY CATTLE 3D {/* 修复按钮文本 */} 
          </button>
        </div>
      </div>
      {/* 右侧视频，和左侧内容等高 */}
      <div className="flex-1 flex justify-center items-center relative" style={{ minHeight: leftHeight ? leftHeight : undefined }}>
        <div className="relative w-full h-full max-w-3xl rounded-3xl overflow-hidden shadow-2xl border-4 border-gray-700 flex items-center justify-center" style={{ height: leftHeight ? leftHeight : 'auto' }}>
          <iframe
            src="https://www.youtube.com/embed/qYF-efFnx9A?autoplay=1&rel=0"
